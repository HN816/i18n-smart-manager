...
class KoreanExtractionService {
  // Vue 파일 처리 함수
  private processVueFile(text: string): string {
    let result = text;
  ...
  private findI18nRanges(text: string): TextRange[] {
    ...
    while ((match = tWithVarsPattern.exec(text)) !== null) {
      ...
      if (/[가-힣]/.test(i18nText)) {
        ...
        // 매칭되는 ']' 찾기
        const arrayEnd = this.findMatchingBracket(text, arrayStart, '[', ']');

        if (arrayEnd !== -1) {
          // 전체 함수 호출의 끝 찾기
          const funcEnd = this.findMatchingBracket(text, start, '(', ')');

          if (funcEnd !== -1) {
            const end = funcEnd + 1;

            // 중복 체크 - 같은 위치에 이미 있는지 확인
            const isDuplicate = i18nRanges.some((range) => range.start === start && range.end === end);
    ...
    while ((match = templateWithVarsPattern.exec(text)) !== null) {
      ...
      if (/[가-힣]/.test(i18nText)) {
        ...
        if (arrayEnd !== -1) {
          // 전체 함수 호출의 끝 찾기
          const funcEnd = this.findMatchingBracket(text, start + 3, '(', ')'); // '{{ ' 다음부터
    ...
    while ((match = quotedWithVarsPattern.exec(text)) !== null) {
      ...
      if (/[가-힣]/.test(i18nText)) {
        ...
        if (arrayEnd !== -1) {
          // 전체 함수 호출의 끝 찾기
          const funcEnd = this.findMatchingBracket(text, start + 1, '(', ')'); // 따옴표 다음부터
    ...
    for (const range of i18nRanges) {
      // 이미 추가된 범위와 겹치는지 확인
      const isOverlapping = uniqueRanges.some((existingRange) => {
        // 겹치는 조건: 범위가 교차하거나 포함되는 경우
      ...
      if (!isOverlapping) {
        ...
        // 겹치는 경우 더 긴 범위로 교체
        const overlappingIndex = uniqueRanges.findIndex(
          (existingRange) => range.start < existingRange.end && range.end > existingRange.start,
        );

        if (overlappingIndex !== -1) {
          const existingRange = uniqueRanges[overlappingIndex];
          const existingLength = existingRange.end - existingRange.start;
          const newLength = range.end - range.start;

          // 더 긴 범위로 교체
          if (newLength > existingLength) {
            uniqueRanges[overlappingIndex] = range;
          }
        }
      }
    }
      ...
      if (char === '"' || char === "'" || char === '`') {
        ...
        if (i < processedText.length && /[가-힣]/.test(content)) {
          ...
          const isInI18nRange = i18nRanges.some((range) => {
            // 디버깅을 위한 로그 (나중에 제거)
            console.log(
              `Checking range: ${originalStart}-${originalEnd} against i18n range: ${range.start}-${range.end}`,
            );
  ...
  public extractKoreanTexts(text: string, fileName: string): ExtractedTexts {
    ...
    if (isVueFile) {
      finalProcessedText = this.processVueFile(processedText);
    }
    ...
    if (isTsFile) {
      ...
      allKoreanRanges = this.findKoreanRangesWithMapping(finalProcessedText, positionMap, []);
    }
